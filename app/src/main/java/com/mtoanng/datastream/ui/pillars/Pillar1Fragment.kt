...
import com.mtoanng.datastream.databinding.FragmentPillar1Binding
...
class Pillar1Fragment : BaseFragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        _binding = FragmentPillar1Binding.inflate(inflater, container, false)
        return binding.root
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        parentVm.pillar1.observe(viewLifecycleOwner) { rows ->
            val first = rows?.firstOrNull() ?: return@observe
            binding.tvScore.text = Formatters.score(first.pillar1Score)
            ...
            binding.tvIdr.text = Formatters.percent(first.idr)
            binding.tvSfri.text = getString(R.string.unit_days, Formatters.score(first.sfri))
            binding.tvHhi.text = Formatters.number(first.hhiSupply)
            binding.tvN1.text = getString(R.string.unit_days, Formatters.score(first.n1Resilience))
            binding.tvUpdated.text = getString(R.string.label_updated_at, Formatters.isoToLocal(first.computedAt))

            // Bar chart of pillar1Score across all regions (max 6 entries)
            val capped = rows.take(6)
            val entries = capped.mapIndexed { idx, dto -> BarEntry(idx.toFloat(), dto.pillar1Score.toFloat()) }
            val labels = capped.map { "${it.regionCode.removePrefix("VN_")}/${it.fuelType.take(3)}" }
            val set = BarDataSet(entries, getString(R.string.pillar_1_short)).apply {
                setColors(intArrayOf(R.color.pillar_1), requireContext())
                valueTextSize = 9f
            }
